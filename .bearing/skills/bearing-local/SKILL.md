...
1. **Orient:** `query({ search_query, task_context, goal, repo: "mynah", limit: 3, max_symbols: 8 })`
2. **Symbol:** `context({ name, repo: "mynah", include_content: false })`
3. **Before edit:** `impact({ target, direction: "upstream", repo: "mynah", summaryOnly: true })`

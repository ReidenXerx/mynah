...
TEST_CASE("the Phase 1 session stubs hold the contract") {
    TmpDir dir;
    EnvOverride override_dir("MYNAH_CONFIG_DIR", dir.path().string());
    mynah_engine* engine = mynah_create(nullptr, nullptr, nullptr, nullptr);
    REQUIRE(engine != nullptr);

    // All succeed without doing anything yet; state stays IDLE until the
    // Phase 2 state machine exists.
    ...
    // push_audio accepts samples (and drops them, for now).
    float samples[16] = {};
    CHECK(mynah_push_audio(engine, samples, 16) == 0);
    CHECK(mynah_push_audio(engine, nullptr, 0) == 0);

    // Null handles are rejected, not crashed on.

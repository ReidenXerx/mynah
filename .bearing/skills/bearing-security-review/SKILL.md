...
2. context({ name: "<entry or sink symbol>", repo: "mynah" })
3. gitnexus_explain({ target: "<file-or-symbol>", repo: "mynah" })
4. gitnexus_pdg_query({ mode: "flows", target: "<function-or-file>", variable: "<inputVar>", repo: "mynah" })
5. gitnexus_pdg_query({ mode: "controls", target: "<function-or-file>", repo: "mynah" })
6. impact({ target: "<changed symbol>", direction: "upstream", mode: "pdg", repo: "mynah" }) when PDG layer exists
7. detect_changes({ scope: "unstaged", repo: "mynah" }) before done
```

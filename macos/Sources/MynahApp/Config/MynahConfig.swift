...
struct MynahConfig: Equatable {
    ...
    // Microphone sensitivity. Defaults match `mynah/config.py` — see the comment
    ...
    /// Honours `MYNAH_CONFIG_DIR` exactly as `mynah/config.py` does, so tests and
    /// alternate profiles work identically from either side.
    static var directory: URL {
        if let override = ProcessInfo.processInfo.environment["MYNAH_CONFIG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: (override as NSString).expandingTildeInPath)
        }
        return FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/mynah")
    }
    ...
    static func loadReporting() -> (config: MynahConfig, error: ConfigReadError?) {
        do {

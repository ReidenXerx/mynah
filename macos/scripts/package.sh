#!/usr/bin/env bash
# Package Mynah.app into a zip you can hand to someone else.
#
...
#
# The build is signed with the local `mynah-dev` identity (see
...
STAGE="$ROOT/build/Mynah-$VERSION"
ARCHIVE="$ROOT/build/Mynah-$VERSION.zip"
...
mynah — voice dictation for macOS
================================

Requires macOS 13 (Ventura) or later, on Apple Silicon.

1. Drag Mynah.app to /Applications.
...
- Logs: log stream --predicate 'subsystem == "com.reidenxerx.mynah"'
TXT
...
# Mynah.app and INSTALL.txt loose into Downloads.
( cd "$ROOT/build" && ditto -c -k --sequesterRsrc --keepParent "Mynah-$VERSION" "$ARCHIVE" )
rm -rf "$STAGE"
...
echo "  xattr -dr com.apple.quarantine /Applications/Mynah.app"

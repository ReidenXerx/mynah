...
if grep -rq 'mynah' .cursor/rules .cursor/hooks .bearing/skills/bearing-workspace .bearing/skills/bearing-enforcement 2>/dev/null; then
  warn "Bundle still references mynah — set GITNEXUS_REPO_NAME and re-run substitution:"
  warn "  GITNEXUS_REPO_NAME=$REPO_NAME bash scripts/bearing-teaching/install-from-bundle.sh"
  if [[ "${GITNEXUS_SKIP_RENAME:-}" != "1" ]]; then
    info "Replacing mynah → $REPO_NAME in rules/hooks/skills"
    find .cursor/rules .cursor/hooks .bearing/skills/bearing-workspace .bearing/skills/bearing-enforcement \
      ...
      -exec sed -i '' "s/mynah/$REPO_NAME/g" {} + 2>/dev/null \
      || find .cursor/rules .cursor/hooks .bearing/skills/bearing-workspace .bearing/skills/bearing-enforcement \
      -type f \( -name '*.mdc' -o -name '*.sh' -o -name '*.mjs' -o -name 'SKILL.md' \) \
      -exec sed -i "s/mynah/$REPO_NAME/g" {} +

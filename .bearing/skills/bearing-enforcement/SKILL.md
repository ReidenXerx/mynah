...
```javascript
gitnexus_query({ search_query: "auth flow", task_context: "...", goal: "...", repo: "mynah", limit: 5, max_symbols: 12 })
gitnexus_context({ name: "<symbol>", repo: "mynah" })
gitnexus_trace({ from: "<source>", to: "<target>", repo: "mynah", maxDepth: 10 })
gitnexus_pdg_query({ mode: "flows", target: "<function-or-file>", variable: "<var>", repo: "mynah" })
gitnexus_explain({ target: "<file-or-symbol>", repo: "mynah" })
READ gitnexus://repo/mynah/schema
gitnexus_cypher({ statement: "MATCH (f)-[r:CodeRelation {type: 'ACCESSES'}]->(p:Property {name: $name}) RETURN f.name, f.filePath, r.reason", params: { name: "<field>" }, repo: "mynah" })
gitnexus_impact({ target: "<symbol>", direction: "upstream", repo: "mynah", summaryOnly: false, limit: 100 })
```

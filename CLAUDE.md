...
Cheap resource reads (prefer before heavy tools): `READ gitnexus://repo/mynah/{context|schema|clusters|processes|process/<name>}`.
...
`npm run bearing:agent-brief` or READ `gitnexus://repo/mynah/context`. Stale or missing embeddings → **`npm run bearing:agent-refresh` first** (`required_permissions: ["all"]`). Hooks **block** Grep/Read/MCP/shell until refresh succeeds; classical tools only if refresh **fails** (say why). Never ask user to analyze.

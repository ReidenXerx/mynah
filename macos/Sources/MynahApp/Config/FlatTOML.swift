...
/// A minimal reader/writer for mynah's flat TOML config.
///
/// mynah's config is deliberately simple — a single table of scalar values and
/// string arrays, no nesting, no dates, no inline tables. The Python side
/// hand-rolls its writer for the same reason (`mynah/config.py:_emit_toml`), so
...
/// `mynah config set` produces, this must read, and whatever this writes,
...
enum FlatTOML {
    ...
    static func parse(_ text: String) -> [String: Value] {
        ...
        while index < lines.count {
            ...
            // vanishes — then `MynahConfig.save()` re-emits only what parsed and

...
/// tests are really compatibility tests against `mynah/config.py`.
@Suite("Flat TOML")
struct FlatTOMLTests {
    ...
    func parsesControlCharEscapes() {
        // mynah/config.py `_escape_toml_string` escapes \n/\r/\t since the
    ...
    func stripsTrailingComments() {
        ...
        // failed to parse and vanished. `MynahConfig.save()` then re-emitted
...
@Suite("MynahConfig", .serialized)
struct MynahConfigTests {

    @Test("defaults match mynah/config.py")
    ...
    /// `MynahConfig.path` honours `MYNAH_CONFIG_DIR`, exactly as `mynah/config.py`
    ...
    private func withIsolatedConfigDir(_ body: () throws -> Void) rethrows {
        let previous = ProcessInfo.processInfo.environment["MYNAH_CONFIG_DIR"]
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mynah-config-tests-\(UUID().uuidString)")
        setenv("MYNAH_CONFIG_DIR", dir.path, 1)
        defer {
            if let previous { setenv("MYNAH_CONFIG_DIR", previous, 1) }
            else { unsetenv("MYNAH_CONFIG_DIR") }
    ...
    func unreadableFileIsReported() throws {
        try withIsolatedConfigDir {
            ...
            try FileManager.default.createDirectory(
                at: MynahConfig.path, withIntermediateDirectories: true)
            let (config, error) = MynahConfig.loadReporting()
            #expect(config == MynahConfig())  // app still works on defaults
            guard case .unreadable(let path, _)? = error else {
                Issue.record("expected an unreadable error, got \(String(describing: error))")
                return
            }
            #expect(path == MynahConfig.path.path)
        }
    }
